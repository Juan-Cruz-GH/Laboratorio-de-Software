...
/**
 * This class represents the board of the game which manage all the items on the
 * board
 */
public class Board extends JPanel {
	...
	JLabel lab1;
	int lives;
	final int W = ContenidoCelda.WALL.getValor(); // Wall.
	final int F = ContenidoCelda.FOOD.getValor(); // Crossroads with food
	final int E = ContenidoCelda.EMPTY.getValor(); // Empty crossroads
	private int board[][] = {
			// -----------------------X-----------------------------//
	...
	/*
	 * Create The StatusBar Score Counter and Lives Figure
	 */
	...
	private void winGameEvent() {
		...
		int result = JOptionPane.showConfirmDialog(this, "Start a new game?", "Way To Go! You Wone The Game",
	...
	public void restart() {
		...
		for (int i = 0; i < 4; i++) {
			ghostSet[i].startingPoint((12 + i) * squareWidth + squareWidth / 2,
					13.5 * (squareHeight) + squareHeight / 2);
			ghostSet[i].rounds = 0;
	...
	private void endGameEvent() {
		...
		int result = JOptionPane.showConfirmDialog(this, "Start a new game?", "Game Over my friend",
				JOptionPane.YES_NO_OPTION);
